...
evoluti::csv_row::csv_row(const std::vector<std::string> &items)
    : items(items) {}

std::vector<std::string> evoluti::csv_row::get_items() const { return items; }
...
std::size_t evoluti::csv_row::num_columns() const { return items.size(); }
